...
export class WeatherService {
  ...
  async getWeather(lat: string, long: string): Promise<WeatherDisplay> {
    try {
      ...
      const currentWeather: any = await this.getCurrentWeatherOpenWeatherMapAPI(
        lat,
        long
      );
      ...
      this.weatherDisplay.currentTemperature = String(
        Math.ceil(currentWeather['main']['temp'])
      );
      this.weatherDisplay.currentCondition =
        currentWeather['weather'][0]['description'];
      this.weatherDisplay.sunrise = this.formatSunrise(
        currentWeather['sys']['sunrise']
      );
      this.weatherDisplay.sunset = this.formatSunset(
        currentWeather['sys']['sunset']
      );

      const detailedForecast = await this.getDetailedForecast(
  ...
  getRadarStation(radarStationURL: string): Promise<any> {
    return this.http
      .get(radarStationURL)
      .toPromise()
  ...
  getLatestObservations(observationsURL: string): Promise<any> {
    return this.http
      .get(observationsURL)
      .toPromise()
  ...
  getHourlyForecast(hourlyURL: string): Promise<any> {
    return this.http
      .get(hourlyURL)
      .toPromise()
  ...
  getDetailedForecast(forecastURL: string): Promise<any> {
    return this.http
      .get(forecastURL)
      .toPromise()
